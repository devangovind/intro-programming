...
class AdminGui:
    ...
    def resource_requests_list(self):
        ...
        for index, row in (self.unresolved[::-1].iterrows()):
            values = [row['Camp_ID'], row['Volunteer'], row['food_pac'], row['medical_sup'], row['tents'],
                      row['date']]
    ...
    def grant_all_requests(self):
        ...
        for index, row in (self.unresolved.iterrows()):
            self.admin.manual_resource_allocation(row['Camp_ID'], str(row['food_pac']), str(row['medical_sup']),
                                                  str(row['tents']))
    ...
    def allocate_resources(self, camp_id, curr_resources, suggest_resources_dict, requested_resources=None):
        ...
        if requested_resources != None:
            requested_strings = [f'Requested Food: {requested_resources[0]}',
                                 f'Requested Medical: {requested_resources[1]}',
                                 f'Requested Tents: {requested_resources[2]}']
        else:
            requested_strings = ["", "", ""]
        ...
        submit_btn = tk.Button(self.submitframe, text="Allocate Edited Resourece", font=('Arial', 20),
                               command=lambda: self.submit_resources(camp_id, self.food_inp.get(),
                                                                     self.med_inp.get(), self.tents_inp.get()))
    ...
    def manage_volunteers(self):
        ...
        activate_all_btn = tk.Button(all_buttons, text="Activate All", font=('Arial', 16),
                                     command=self.activate_all)
        deactivate_all_btn = tk.Button(all_buttons, text="Deactivate All", font=('Arial', 16),
                                       command=self.deactivate_all)
        activate_all_btn.grid(row=0, column=0)
        deactivate_all_btn.grid(row=0, column=1)
        all_buttons.pack()
        info_lbl = tk.Label(self.root, text="Click on a State or Delete column to edit an individual volunteer",
                            font=('Arial', 16))
        info_lbl.pack()
        camp_columns = ["Camp ID", "Username", "First Name", "Surname", "Phone", "Age", "Availability", "State",
                        "Delete"]
    ...
    def filter_volunteers(self, event=None):
        ...
        if selected_camp == "All Camps":
            filtered_data = self.volunteer_data
        else:
        ...
        if not filtered_data.empty:
            for index, row in (filtered_data.iterrows()):
                ...
                if state:
                    ...
                    state = "Deactive"
                values = [row['Camp_ID'], row['Username'], row['First Name'], row['Last Name'], row['Phone'],
                          row['Age'], availability_string, state, "Delete?"]
    ...
    def activate_deactivate(self, row):
        username = row[1]
        if row[-2] == "Active":
            volunteer_choice = messagebox.askyesno(title="Manage Volunteer",
                                                   message=f"Deactivate {username}'s Account:\n")
            ...
            volunteer_choice = messagebox.askyesno(title="Manage Volunteer",
                                                   message=f"Activate {username}'s Account:\n")
            if volunteer_choice == True:
